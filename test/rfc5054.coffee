...
A_expected = transform.cleanHex A_expected
...
B_expected = transform.cleanHex B_expected
...
S_expected = transform.cleanHex S_expected

describe 'RFC 5054', ->
    it 'initialize client', (done) ->
        client.debugInit
    ...
    it 'v', ->
        result = client.srp.v I: I, P: P, salt: s
        result = transform.buffer.toHex result
        result.should.equal v_expected

    it 'x', ->
        client.setSalt(s.toString('hex'));
        result = transform.bigInt.toHex client.xInt
        result.should.equal x_expected

    it 'A', ->
        client.setServerPublicKey(server.getPublicKey());
        client.getPublicKey().should.equal A_expected

    it 'B', ->
        server.setClientPublicKey(client.getPublicKey());
        server.getPublicKey().should.equal B_expected

    it 'u', ->
        clientU = transform.bigInt.toHex client.uInt
        clientU.should.equal u_expected

    it 'S client', ->
        clientS = transform.buffer.toHex client.SBuf
        clientS.should.equal S_expected

    it 'S server', ->
        serverS = transform.buffer.toHex server.SBuf
        serverS.should.equal S_expected
